...
async function main() {
  ...
  const outputPath = path.join(__dirname, "./deployLogs/" + new Date().getTime().toString() + `_factory_deployed.json`);
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 4));

  const overwriteOutputPath = path.join(__dirname, `factory_deployed.json`);
