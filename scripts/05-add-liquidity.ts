...
async function main() {
  ...
  const TOKEN_USDC = process.env.TOKENS_USD;
  const TOKEN_EURS = process.env.TOKENS_EUR;
  ...
  console.log(formatCurrency(TOKEN_NAME.USDC, TOKEN_DECIMAL.USDC, await usdc.balanceOf(_deployer.address)));
  console.log(formatCurrency(TOKEN_NAME.EURS, TOKEN_DECIMAL.EURS, await eurs.balanceOf(_deployer.address)));
