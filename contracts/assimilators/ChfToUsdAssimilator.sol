...
contract ChfToUsdAssimilator is IAssimilator {
    ...
    // takes a numeraire account, calculates the raw amount of chf, transfers it in and returns the corresponding raw amount
    function intakeNumeraireLPRatio(
        ...
        // Rate is in 1e6
        uint256 _rate = _usdcBal.mul(DECIMALS).div(_chfBal.mul(DECIMALS).div(_baseWeight));
        ...
        returns (int128 amount_, int128 balance_)
    {
        ...
        uint256 _chfAmount = ((_amount).mul(_rate)).div(1e8);
    ...
    function viewNumeraireBalanceLPRatio(
        ...
        balance_ = ((_chfBal.mul(_rate)).div(1e6)).divu(DECIMALS);
    }
}
